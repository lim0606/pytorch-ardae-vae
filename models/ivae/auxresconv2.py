...
from models.vae.auxresconv import InputEncoder, AuxEncoder, AuxDecoder
from models.vae.auxresconv import Encoder as _Encoder
...
def sample_gaussian(mu, logvar, _std=None, eps=None, min_std=0.):
    if _std is None:
        _std = 1.
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.encode = _Encoder(z0_dim=z0_dim, z_dim=z_dim, c_dim=c_dim, act=nn.ELU(), do_center=do_center)
    ...
    def _forward_w_eps(self, input, std=None, nz=1, eps0=None, eps=None):
        ...
        # aux encode
        _, mu_qz0, logvar_qz0 = self.aux_encode(inp, _std=std)
        ...
        z0, _ = sample_gaussian(mu_qz0, logvar_qz0, _std=std, eps=eps0, min_std=1.)#.view(batch_size*nz, -1)

        # encode
        _, mu_qz, logvar_qz, h = self.encode(inp, z0, nz=nz)
        ...
        # sample z
        z, _ = sample_gaussian(mu_qz, logvar_qz, _std=std, eps=eps, min_std=0.)
...
class ImplicitPosteriorVAE(nn.Module):
    def __init__(self,
        ...
        self.encode = Encoder(input_height, input_channels, z0_dim=z0_dim, z_dim=z_dim, c_dim=c_dim, nonlinearity=nonlinearity, do_center=do_center)
