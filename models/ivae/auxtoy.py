...
from models.vae.auxtoy import AuxEncoder, AuxDecoder
from models.vae.auxtoy import SimpleEncoder

from utils import expand_tensor
from utils import cond_jac_clamping_loss


#def sample_noise(batch_size, input_size, device):
#    eps = torch.randn(batch_size, input_size).to(device)
#    return eps
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.aux_encode = AuxEncoder(input_dim, h_dim, noise_dim, nonlinearity=nonlinearity, num_hidden_layers=num_hidden_layers, clip_logvar=clip_z0_logvar)
        if enc_type == 'simple':
            self.encode = SimpleEncoder(input_dim, noise_dim, h_dim, z_dim, nonlinearity=nonlinearity, num_hidden_layers=num_hidden_layers, clip_logvar=clip_z_logvar)
        else:
    ...
    def _forward_w_eps(self, input, std=None, nz=1, eps0=None, eps=None):
        ...
        # gen eps0, eps
        if eps0 is None:
            eps0 = sample_noise(sz=[batch_size*nz, self.noise_dim], device=input.device)
        if eps is None:
            eps = sample_noise(sz=[batch_size*nz, nz, self.z_dim], device=input.device)
        ...
        # expand
        mu_qz = convert_2d_3d_tensor(mu_qz, sample_size=nz)
        logvar_qz = convert_2d_3d_tensor(logvar_qz, sample_size=nz)

        # sample z
        z, _ = sample_gaussian(mu_qz, logvar_qz, _std=std, eps=eps)
        z = z.view(batch_size, nz*nz, -1)
...
class ImplicitPosteriorVAE(nn.Module):
    def __init__(self,
        ...
        self.encode = Encoder(input_dim, noise_dim, h_dim, z_dim, nonlinearity=nonlinearity, num_hidden_layers=num_hidden_layers, enc_type=enc_type, clip_z0_logvar=clip_z0_logvar, clip_z_logvar=clip_z_logvar)
        self.decode = Decoder(input_dim, h_dim, z_dim, nonlinearity=nonlinearity, num_hidden_layers=num_hidden_layers, init=init)
        #self.aux_decode = AuxDecoder(input_dim, z_dim, noise_dim, h_dim, nonlinearity=nonlinearity, num_hidden_layers=num_hidden_layers, clip_logvar='spm4')
    ...
    #def aux_loss(self, mu_qz0, logvar_qz0, mu_pz0, logvar_pz0):
    #    # aux dec loss: -log r(z0|z,x)
    #    aux_kld_loss = loss_kld_gaussian_vs_gaussian(
    #            mu_qz0.detach(), logvar_qz0.detach(),
    #            mu_pz0, logvar_pz0,
    #            do_sum=False,
    #            )
    #    return aux_kld_loss
    ...
    def aux_loss(self, mu_pz0, logvar_pz0, target_z0):
        # aux dec loss: -log r(z0|z,x)
        aux_recon_loss = loss_recon_gaussian(mu_pz0, logvar_pz0, target_z0.view(-1, self.noise_dim), do_sum=False)
    ...
    def loss(self,
             ...
             mu_px, logvar_px, target_x,
             beta=1.0,
             ):

        # primary loss to train model
        recon_loss, prior_loss = self.primary_loss(z, mu_px, logvar_px, target_x)
    ...
    def forward_hidden(self, input, std=None, nz=1):
        ...
        (z0, mu_qz0, logvar_qz0, eps0,
         z, mu_qz, logvar_qz, eps,
         _,
         ) = self.encode._forward(input, std=std, nz=_nz)
    ...
    def forward(self, input, beta=1.0, eta=0.0, lmbd=0.0, std=None, nz=1):
        ...
        # z flattten
        z0_expanded_flattened = convert_2d_3d_tensor(z0, sample_size=_nz).view(batch_size*nz, -1)
        ...
        # return
        return x, mu_px, z, loss, recon_loss.detach(), prior_loss.detach() #,aux_recon_loss.detach()
    ...
    #    ''' get - (log q(z|z0,x) + log q(z0|x) - log p(z0|z,x) - log p(z)) '''
    #    ''' get log q(z0|x) '''
    ...
    #    z0 = self.encode.aux_encode.sample(mu_qz0, logvar_qz0) # bsz*ssz1 x noise_dim
    ...
    #    ''' get log q(z|z0,x) '''
    #    # forward
    #    nos = self.encode.encode._forward_nos(z0) # bsz*ssz1 x noise_dim
    #    inp = self.encode.encode._forward_inp(input) # bsz x noise_dim
    ...
    #    #_, mu_qz, logvar_qz, _ = self.encode.encode._forward_all(inp, nos) # bsz*ssz1 x z_dim
